...
\item The list of formats may contain either integers or format types (\textbf{halfprecision},
   \textbf{single}, \textbf{double}, \textbf{doubledouble}, \textbf{tripledouble}, \textbf{doubleextended} and \textbf{quad}). The
   list may be too large or even infinite. Only the first indications will be
   considered. For instance, for a degree $n$ polynomial, $\mathrm{formats}[n+1]$
...
\item The floating-point coefficients considered by \textbf{fpminimax} do not have an
   ...
   for 53. Currently, \textbf{fpminimax} only ensures that the corresponding coefficient has
