...
\title{Users' manual for the \sollya tool \\ {\large Release 2.0 alpha} \\ ~ \\ {\large \'Equipe-projet Ar\'enaire\\Laboratoire de l'Informatique du Parall\'elisme \\ UMR CNRS - ENS Lyon - UCB Lyon 1 - INRIA 5668}}
...
The \sollya tool is copyright \copyright~ 2008-2010 Laboratoire de
...
payloads, signaling or quiet NaNs or signs of NaNs. Signalling NaNs
...
cases. Howevert, particular commands might prohibit their use even
...
\subsection{Rational numbers and rational arithmetic}
...
Even when \com{rationalmode} is \com{on}, \sollya will not able to
...
%% TODO Add paragraph on interval arithmetic supported natively.
%% TODO Add paragraph on thin intervals defined by one number [x], 
%% including their displaying in midpointmode.
%% TODO Passer sur les aides des fonctions plus, moins, fois, 
%% diviser et puissance: c'est des fonctions bivariées avec un 
%% comportement particulier pour ce qui est des intervalles.
%% TODO Rajouter une explication pour la syntaxe [x].
%% TODO Dire que pour les intervalles meme si affiche un warning, la propriete 
%% d'inclusion est verifiee; meme si l'arrondi n'est pas faithful: 
%% exemple [log(3)/log(17) - log2(3)/log2(17)]
%% Ce n'etait pas comme ca dans Sollya version 1.1
%% TODO Dire que meme en mode rationalmode, les bornes d'intervalles restent des flottants.
%% Il y a soit le mode rationalmode avant les intervalles ou les intervalles avant 
%% le rationalmode.
...
evaluation bounds defined by constant expressions include $\pi$:
...
the case by procedures with an arbitrary number of actual arguments.
