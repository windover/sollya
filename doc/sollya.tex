...
\sollya intervals are generally closed subsets of the real
numbers. Two ways of defining intervals exist in \sollya. There is the
...
values are separated either by commas or semi-colons:
...
bounds. Interval bound evaluation is performed in a way that ensures
the inclusion property: all points in the original, unevaluated
interval will be contained in the interval with its bounds evaluated
...
containing infinities or NaNs. When evaluation yield an interval with
...
interval arithmetic in order to compute output intervals that safe
...
evaluator will always guarantee the inclusion property, it applies
...
element of the list using indices is $\mathcal{O}(n)$.
