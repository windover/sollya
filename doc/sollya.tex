...
The user should be aware that the function shows a particular behavior in the case when the Sollya object \verb|f| is a library constant, a library function or a procedure function. This particular behavior allows the actual library constant, library function or procedure function to be recovered somehow. In this case, the function (resp. constant) itself, $f_0$, is stored at the address \texttt{g\_t} provided after the addresses \texttt{g\_1} thru \texttt{g\_s}. As a matter of course, the object is only stored if the argument list is sufficiently long until \texttt{NULL} is encountered. The user should additionnally be aware that in this particular case, the number of Sollya library objects \texttt{g\_i} returned is one more than the arity of the function stored at the address pointed by \texttt{n}.
