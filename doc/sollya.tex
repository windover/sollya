...
\end{itemize}
Except for a few functions for which it is explicitly specified, the following conventions are used:
\begin{itemize}
...
\item Second, while \verb|sollya_lib_build_function_foo| mechanically constructs an expression, \verb|sollya_lib_foo| also evaluates it, as far as it is possible without performing a rounding.\\
...
Three cases are possible. In all cases, the function creates a fresh object and stores it at the address referred by \texttt{res}.
\begin{itemize}
...
\end{itemize}
Please note that \texttt{s} is not changed by this function: the structure stored in \texttt{*res} is a new one that does not refer to any of the components of \texttt{s}. As a consequence, one should not forget to explicitly clear \texttt{s} as well as \texttt{*res} when they become useless.

\subsection{Getting the type of an object}
Functions are provided that allow the users to test the type of a \sollya object. They are listed in Table~\ref{type_of_an_object}. They all return an \verb|int| interpreted as the boolean result of the test. Please note that from a typing point of view, a mathematical constant and a non-constant functional expression are both functions.
...
The function  \verb|sollya_lib_get_prec_of_constant(mp_prec_t *prec, sollya_obj_t arg)| tries to find a precision that would be sufficient to exactly represent the value of \verb|arg| without rounding. If it manages to find such a precision, it stores it in \verb|*prec| and returns an \verb|int| which boolean value is true. If it does not manage to find such a precision, or if \verb|arg| is not a constant expression, it returns false and \verb|*prec| is left unchanged. In conclusion, here is an example of use for converting a constant expression to a \verb|mpfr_t|:
...
\begin{itemize}
\item If \verb|arg| is a regular list of length $N$, the function allocates memory on the heap for $N$ \verb|sollya_obj_t|, sets \verb|*L| so that it points on that memory segment, and copies each of the elements $N$ of \verb|arg| at \verb|(*L)[0]|, \dots, \verb|(*L)[N-1]|. Finally, it sets \verb|*n| to $N$, \verb|*end_ell| to $0$ and returns an \verb|int| which boolean value is true. A particular case is when \verb|arg| is the empty list: everything is the same except that no memory is allocated and \verb|*L| is left unchanged.
\item If \verb|arg| is an end-elliptic list containing $N$ elements plus the ellipse. The function allocates memory on the heap for $N$ \verb|sollya_obj_t|, sets \verb|*L| so that it points on that memory segment, and copies each of the elements $N$ of \verb|arg| at \verb|(*L)[0]|, \dots, \verb|(*L)[N-1]|. Finally, it sets \verb|*n| to $N$, \verb|*end_ell| to $1$ and returns an \verb|int| which boolean value is true. The only difference between a regular list and an end-elliptic list containing the same elements is hence that \verb|*end_ell| is set to $1$ in the latter.
\item If \verb|arg| is neither a regular nor an end-elliptic list, \verb|*L|, \verb|*n| and \verb|*end_ell| are left unchanged and the function returns an \verb|int| which boolean value is false.
\end{itemize}
...
\begin{tabular}{|l|p{7cm}|}
  ...
  \verb|SOLLYA_FP_OBJ_NO_FUNCTION| & \verb|f| is not a functional expression.\\[0.3cm]
  \verb|SOLLYA_FP_FAITHFUL| & The algorithm ended up at step (a) with a finite value.\\[0.3cm]
  \verb|SOLLYA_FP_BELOW_CUTOFF| & The algorithm ended up at step (b).\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_ZERO_CONTAINED_BELOW_THRESHOLD| & The algorithm ended up at step (c) and $I$ was an interval of the form $[-\delta_1,\,\delta_2]$ where the $\delta_i$ are positive and very small (below some threshold of the algorithm). This typically happens when $f(a)$ exactly equals zero, but the algorithm does not manage to prove this exact equality.\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_ZERO_CONTAINED_NOT_BELOW_THRESHOLD| & The algorithm ended up at step (c) with an interval $I$ containing $0$ but too big to fall in the above case. In general, this should be considered as a case of failure and the value stored in \verb|res| might be completely irrelevant.\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_ZERO_NOT_CONTAINED| & The algorithm ended up at step (c) with an interval $I$ that does not contain $0$. In general, this should be considered as a case of failure and the value stored in \verb|res| might be completely irrelevant.\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_INFINITY_CONTAINED| & The algorithm ended up at step (c) and (at least) one of the bounds of $I$ was infinite. This typically happens when the limit of $f(x)$ when $x$ goes to $a$ is infinite.\\[0.3cm]
  ...
  \verb|SOLLYA_FP_FAILURE| & The algorithm ended up at step (c) and $I$ contained NaN. This typically happens when $a$ is not in the definition domain of $f$.\\[0.3cm]
  \verb|SOLLYA_FP_CUTOFF_IS_NAN| & \verb|cutoff| was not \verb|NULL| but the value of \verb|*cutoff| is NaN.\\[0.3cm]
  \verb|SOLLYA_FP_EXPRESSION_NOT_CONSTANT| & \verb|a| is not a constant expression.\\
\hline
  \end{tabular}
\end{table}
...
This function returns an \verb|int| interpreted as a boolean: false means failure (i.e., \verb|f| is not a functional expression), in which case \verb|res| is left unchanged, and true means success, in which case \verb|res| contains the result of the evaluation. The function might succeed, and yet \verb|res| might contain something useless such as an unbounded interval or even $[\textrm{NaN},\textrm{NaN}]$ (this happens for instance when \verb|a| contains points that lie in the interior of the complement of the definition domain of \verb|f|). It is the responsibility of the user to check afterwards whether the computed interval is bounded, unbounded or NaN.
...
\label{callbacks}
The philosophy of \sollya is ``whenever something is not exact, mention it explicitly''. This is a nice feature since this ensures that the users always perfectly know the degree of confidence they can have in a result (is it exact? or only faithful? or even purely numerical, without any warranty?) However, it is sometimes desirable to hide some (or all) of these messages. This is especially true in library mode where messages coming from \sollya are intermingled with the messages of the main program. The library hence provides a specific mechanism to catch all messages emitted by \sollya and handle each of them specifically: installing a callback for messages.
...
\item If the environment variable \verb|roundingwarnings| is set to \verb|on| and if the message informs the user that a rounding occurred, it is filtered.
...
It is possible to get the currently installed callback using \verb|sollya_lib_get_msg_callback()|. This function returns a pointer to the current callback --- i.e., something of type \verb|int (*)(sollya_msg_t)| --- if a callback is installed, or \verb|NULL| if no callback is currently installed.
...
\end{itemize}
In the future, other accessors could be added (to get the verbosity level at which the message has been emitted, to get data associated to the message, etc.) The developers of \sollya are open to suggestions and feature requests on this subject.

As an illustration we give a few examples of possible use of callbacks:
...
\begin{minipage}{15cm}\begin{Verbatim}[frame=single]
...
int (*)(int) old_callback = sollya_lib_get_msg_callback();
...
More involved examples are possible: for instance, instead of setting a flag, it is possible to keep in some variable what the last message was. One may even implement a stack mechanism and store the messages in a stack, in order to handle them later. One may also decide to raise an exception on a a particular message, etc.
\end{document}
