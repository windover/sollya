...
Further, there is another way of displaying formatted strings containing \sollya objects, using a {\tt printf}-like syntax. Four functions are provided, namely \verb|sollya_lib_printf|, \verb|sollya_lib_v_printf|, \verb|sollya_lib_fprintf|, \verb|sollya_lib_v_fprintf|. Each one of these functions overloads the usual function (respectively, \verb|printf|, \verb|vprintf|, \verb|fprintf| and \verb|vfprintf|). The full syntax of conversions specifiers supported with the usual functions is handled (please note that the style using '\verb|$|' ---~as in \verb|%3$| or \verb|*3$|~--- is not handled though. It is not included in the C99 standard anyway). Additionally, the following conversion specifiers are provided:
\begin{itemize}
...
\begin{itemize}
\item \verb|sollya_obj_t sollya_lib_list(sollya_obj_t[] L, int n)|: this function returns a new object that is a list the elements of which are copies of \verb|L[0]|, \dots, \verb|L[n]|.
...
\item If $I$ contains at most one floating-point number of precision $q$:
  \begin{enumerate}
  \item Then set \verb|res| to that number (or set it to any of the two enclosing numbers, if $I$ does not contain any number of precision $q$) and return.
  \item Else if all numbers in $I$ are smaller than $\varepsilon$ in absolute value, then set \verb|res| to $0$ and return .
  \item Else if $p$ has already been increased many times, then set \verb|res| to the middle of $I$ and return.
...
\begin{tabular}{|l|p{7cm}|}
  ...
  \verb|SOLLYA_FP_PROVEN_EXACT| & The algorithm ended up at step (a) and $I$ was equal to $[x,x]$ where $x$ was a finite floating-point number at precision~$q$.\\[0.3cm]
  \verb|SOLLYA_FP_CORRECTLY_ROUNDED_PROVEN_INEXACT| & The algorithm ended up at step (a) with a finite value. $I$ was equal to $[x,y]$ with $\textrm{RN}(x) = \textrm{RN}(y) \not\in I$.\\[0.3cm]
  \verb|SOLLYA_FP_CORRECTLY_ROUNDED| & The algorithm ended up at step (a) with a finite value. $I$ was equal to $[x,y]$ with $\textrm{RN}(x) = \textrm{RN}(y) \in I$.\\[0.3cm]
  \verb|SOLLYA_FP_FAITHFUL_PROVEN_INEXACT| & The algorithm ended up at step (a) with a finite value, and $I$ did not contain any floating-point number at precision $q$.\\[0.3cm]
  \verb|SOLLYA_FP_FAITHFUL| & The algorithm ended up at step (a) with a finite value. This does not say anything about the exactness of the result, though it might have been proved exact or inexact if the algorithm had been run with a larger precision.\\[0.3cm]
  \verb|SOLLYA_FP_BELOW_CUTOFF| & The algorithm ended up at step (b).\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_ZERO_CONTAINED_BELOW_THRESHOLD| & The algorithm ended up at step (c) and $I$ was of the form $[-\delta_1,\,\delta_2]$ where $0 < \delta_i \ll 1$ (below some threshold of the algorithm). This typically happens when $f(a)$ exactly equals zero, but the algorithm does not manage to prove this exact equality.\\[0.3cm]
  \verb|SOLLYA_FP_NOT_FAITHFUL_ZERO_CONTAINED_NOT_BELOW_THRESHOLD| & The algorithm ended up at step (c) with an interval $I$ containing $0$ but too large to fall in the above case. In general, this should be considered as a case of failure and the value stored in \verb|res| might be completely irrelevant.\\[0.3cm]
...
It is possible to come back to the default behavior, using \verb|sollya_lib_uninstall_msg_callback()|. In particular, if a callback is installed and one wants to install another one, one should always uninstall the first callback and, only then, install the new one.
...
\item \verb|char *sollya_lib_msg_to_text(sollya_msg_t msg)| returns a generic string briefly summarizing the content of the message. Please note that this \verb|char *| is dynamically allocated on the heap and should manually be cleared with \verb|sollya_lib_free| when it becomes useless.
\end{itemize}
...
In order to solve this issue, \sollya provides a chaining mechanism that we are now going to describe. The idea is the following: suppose that the main program should use a function \verb|custom_malloc|. The user should not use \verb|mp_set_memory_functions| as usual, but should instead initialize \sollya with the initializing function described above. This will cause \sollya to register an allocation function \verb|sollya_lib_malloc| to \verb|GMP|. This function overloads \verb|custom_malloc|: when called, it first does its own stuff and then terminates by calling \verb|custom_malloc| which performs the actual allocation. The pointer returned by \verb|sollya_lib_malloc| is exactly the one returned by \verb|custom_malloc|. Hence, from the point of view of the user, the mechanism is completely transparent and equivalent to directly registering \verb|custom_malloc| to \verb|GMP|. The same holds for all other allocation functions: in particular, this is true for \verb|free| as well: if a function \verb|custom_free| is given at the initialization of \sollya, then the function \verb|sollya_lib_free| eventually uses \verb|custom_free| to free the memory. 
...
\end{center}
None of the arguments is mandatory: if the user does not want to provide an argument, they may use \verb|NULL| as a placeholder for that argument. In that case, the default corresponding \sollya function will be used. Indeed, the default initializing function \verb|sollya_lib_init()| is just an alias to \verb|sollya_lib_init_with_custom_memory_functions(NULL, NULL, NULL, NULL, NULL, NULL)|.
