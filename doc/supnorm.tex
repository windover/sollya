...
\item \textbf{supnorm}(\emph{p}, \emph{f}, \emph{I}, \emph{errorType}) tries to compute an interval
   ...
   such that $\sup_{x \in I} \{|\varepsilon(x)|\}\subseteq r$ and $0 \le \left \vert\frac{u}{\ell} - 1 \right \vert \le {\textrm{accuracy}}$. 
...
\item Roughly speaking, \textbf{supnorm} is based on \textbf{taylorform} to obtain a higher
   ...
   an a posteriori validation of a potential supremum norm upper bound by
   a validated way of showing that a certain polynomial the problem gets
