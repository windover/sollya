...
<span class="smallDescription">computes a rigorous polynomial approximation (polynomial, interval error bound) for a function, based on Chebyshev interpolants. 
</span> 
</div> 
...
for function f. More precisely, it returns a list L=[p, coeffErrors, Delta, chebCoeffs] 
where: 
<ul> 
...
</li><li> chebCoeffs is a list of n+1 intervals. These are the interval coefficients obtained for the representation of interpolation polynomial considered in Chebyshev basis. See details below. 
</li></ul> 
</li><li>The polynomial p and the bound  Delta are obtained using Chebyshev Models 
principles. 
</li><li>More formally, the mathematical property ensured by the algorithm may be 
stated as follows: 
<br> 
Using monomial basis representation given by p: there exist (small) values eps[i] in coeffsErrors[i] such that: 
for all x in I, there exists delta in Delta such 
that f(x) - p(x) = sum{i=0...n} eps[i]*x^i + delta. 
<br> 
Using Chebyshev basis representation given by chebCoeffs: there exist values alpha[i] in chebCoeffs[i] such that: 
 for all x in I, there exists delta in Delta such 
that f(x) - sum{i=0...n} alfa[i]*T_i(y) =delta, where [a,b]=I, y=(2*x-b-a)/(b-a) and  T_i(y) is the ith Chebyshev polynomial over  [-1,1]. 
