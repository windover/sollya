...
int sturm_mpq(int *n, mpq_t *p, int p_degree, mpfi_t x){
  ...
  polynomialEval_mpq( &evalResA[evalNr], aq, s0, s0_degree);
  polynomialEval_mpq( &evalResB[evalNr], bq, s0, s0_degree);
  ...
  for (i=1; i<evalNr; i++){
    if ((mpq_cmp_ui(evalResA[i-1],0,1) * mpq_cmp_ui(evalResA[i],0,1))<0) 
      varSignA++;
    if (((mpq_cmp_ui(evalResA[i-1],0,1) * mpq_cmp_ui(evalResA[i],0,1))==0)&&  ((mpq_cmp_ui(evalResA[i-1],0,1)<0) || (mpq_cmp_ui(evalResA[i],0,1)<0)))  
      varSignA++;
  } 
  varSignB=0;
  for (i=1; i<evalNr; i++){
    if ((mpq_cmp_si(evalResB[i-1],0,1) * mpq_cmp_si(evalResB[i],0,1))<0) 
      varSignB++;
    if (((mpq_cmp_ui(evalResB[i-1],0,1) * mpq_cmp_ui(evalResB[i],0,1))==0)&&  ((mpq_cmp_ui(evalResB[i-1],0,1)<0) || (mpq_cmp_ui(evalResB[i],0,1)<0)))  
      varSignB++;
  }
  //printf("\nA variations: = %d\n", varSignA);
  //printf("\nB variations: = %d\n", varSignB);
  *n=(((varSignA-varSignB)>0)?(varSignA-varSignB):(varSignB-varSignA) );
return 1;
}
