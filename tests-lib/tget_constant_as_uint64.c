...
int main(void) {
  ...
  res = -17;
  a = SOLLYA_CONST(1);
  for (i=1;i<=sizeof(int64_t)*8;i++) a = SOLLYA_MUL(a, SOLLYA_CONST(2));
  ...
  if (!sollya_lib_get_constant_as_uint64(&res, a))
    ...
    sollya_lib_printf("%b has been converted to %" PRIu64 " (ideally inf, but any value would do the trick, provided that a warning message be displayed.)\n\n", a, res);
  }
  sollya_lib_clear_obj(a);
 ...
 /* Trying NaN */
  ...
  if (!sollya_lib_get_constant_as_uint64(&res, a))
    ...
    sollya_lib_printf("%b has been converted to %" PRIu64 " (expecting NaN).\n\n", a, res);
  }
  sollya_lib_clear_obj(a);
