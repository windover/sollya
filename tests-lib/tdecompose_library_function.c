...
int main(void) {
  ...
  res = sollya_lib_decompose_library_function(&libfun, &deriv, &f[4], f[3]);
  if (res) sollya_lib_printf("sollya_lib_decompose_library_function should not succeed on %b but it did.\n", f[3]);
  ...
  deriv = -1; libfun = NULL;
  res = sollya_lib_decompose_library_function(&libfun, &deriv, &f[6], f[5]);
  ...
  if (!res) sollya_lib_printf("sollya_lib_decompose_library_function should succeed on %b. It did not.\n", f[5]);
  else {
  ...
  deriv = -1; libfun = NULL;
  res = sollya_lib_decompose_library_function(&libfun, &deriv, &f[17], f[16]);
  ...
  deriv = -1; libfun = NULL;
  res = sollya_lib_decompose_library_function(&libfun, &deriv, &f[29], f[28]);
