...
\item \com{arenaireplot} comprend aussi des opérateurs fonctionnels
  ...
  \begin{itemize}
    ...
    \item Il est possible de lier des fonctions implémentées à
      l'exterieur d'\com{arenaireplot} en \com{MPFI} à des symboles de
      fonction d'\come{arenaireplot}. Ensuite, il est possible d'utiliser ces
      fonctions comme les fonctions prédéfinies comme par exemple \com{exp} ou
      \com{sin}. Pour cela, il est necessaire de disposer d'une
      bibliothèque dynamique qui déclare une fonction du type {\tt int
      name(mpfi\_t rop, mpfi\_t op, int n)}. La commande \com{name =
      library(chaine);} lie alors la fonction \com{name} de la
      bibliothèque dont le nom est spécifié par \com{chaine} au
      symbole de fonction \com{name}. La fonction \com{name} peut dans
      la suite être utilisée comme tout autre opérateur. Pour évaluer
      la fonction \com{name} ainsi que ses dérivées,
      \com{arenaireplot} appelle la fonction de la bibliothèque
      liée. Celle-ci est censée renvoyer dans \com{rop} un encadrement
      de l'image de la \com{n}-ième derivée de la fonction sur le
      domaine \com{op}. Remarquons que le comportement
      d'\com{arenaireplot} concernant la gestion de la précision ainsi
      que de la correction des algorithmes dépend alors complètement
      de la bibliothèque extérieure. Celle-ci est censée ``réagir'' à
      la précision de \com{rop}.  Remarquons également que si une
      fonction liée à une bibliothèque est déréférencée avant que la
      variable globale libre soit liée à un nom, cette dernière est
      liée au nom \com{x} avant la déréférentiation.
  \end{itemize}
\end{itemize}
